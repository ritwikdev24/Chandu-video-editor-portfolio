...
Premium portfolio website for Senior Video Editor Purna Chandra Koppisetti. Built on TanStack Start (not Next.js) with a cinematic dark theme.
...
src/
  routes/          # TanStack file-based routes
    ...
    admin.tsx      # Admin dashboard with login
  ...
  components/
    ...
    LoadingScreen.tsx  # Cinematic loading with progress bar
    AnimatedCursor.tsx # Custom dual-element cursor (desktop only)
    ScrollProgress.tsx # Fixed top bar tracking scroll %
    BackToTop.tsx      # FAB shown after 400px scroll
    sections/
      Hero.tsx          # Canvas particle hero section
      About.tsx         # Stats with count-up animation (useInView)
      Skills.tsx        # Progress bar cards (useInView trigger)
      Experience.tsx    # Alternating vertical timeline
      Services.tsx      # 3-col service card grid
      Portfolio.tsx     # Filterable grid + YouTube modal
      Testimonials.tsx  # 3-col glass testimonial cards
      Contact.tsx       # Split layout: info + form → /api/contact

db/
  schema.ts     # Drizzle ORM table definitions
  index.ts      # DB client via drizzle-orm/netlify-db

netlify/
  functions/
    contact.ts          # POST /api/contact → saves to contact_messages
    admin-messages.ts   # GET /api/admin/messages
    portfolio-api.ts    # CRUD /api/portfolio
  database/
...
- All components use inline Tailwind classes — no separate CSS modules
- Global CSS utilities defined in src/styles.css (@layer utilities): .glass-card, .gradient-gold, .btn-gold, .btn-outline, .section-label, .section-title
- Color palette: #080808 bg, #0d0d0d alt-section bg, #c9a84c gold, #e8c97e gold-light
- Fonts: Montserrat (body) + Cormorant Garamond (headings/quotes) from Google Fonts
- All animations via Framer Motion whileInView with viewport once:true
- Sections receive id attributes matching nav href anchors (#home, #about, etc.)

## Database Schema

- portfolio_videos — video cards for the portfolio section
- testimonials — testimonial cards (currently static, DB-ready)
- contact_messages — form submissions from the contact section
- admin_users — reserved for future auth upgrade

## Admin Panel

- Route: /admin
- Auth: localStorage key pck_admin_auth, password PCK@admin2026
- Tabs: Dashboard (stats), Portfolio (CRUD videos), Messages (DB), Settings

## Non-Obvious Decisions

- Template routes (blog, contact, projects, resume) from the portfolio scaffold were removed entirely since this is a single-page portfolio
- AnimatedCursor guards against SSR/mobile with typeof window check
- Canvas particle system in Hero uses RAF loop; cleaned up on unmount
- Skills progress bars use a two-step trigger: useInView sets boolean, then useEffect sets CSS width to animate
- Admin password is intentionally stored as a plain string constant — not a real auth system, just a basic gate
